  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.infoRow}>
        <MaterialIcons name="directions-car" size={20} color="#687076" style={styles.icon} />
        <LabelText>ETA</LabelText>
        <Text>{formatRelative(job.arrivalTime)}</Text>
      </View>
      
      <View style={styles.infoRow}>
        <MaterialIcons name="schedule" size={20} color="#687076" style={styles.icon} />
        <LabelText>Arrival</LabelText>
        <Text>{formatDateTime(job.arrivalTime)}</Text>
      </View>
      
      <View style={[styles.infoRow, styles.marginBottom]}>
        <MaterialIcons name="location-pin" size={20} color="#687076" style={styles.icon} />
        <LabelText>Address</LabelText>
        <Text numberOfLines={2} style={styles.addressText}>{job.Address?.short}</Text>
      </View>
    </View>
  );
}
