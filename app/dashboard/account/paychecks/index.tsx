...
export default function PaychecksScreen() {
  ...
  const fetchPaychecks = useCallback(async (page = 1) => {
    if (!userId) {
      setLoading(false);
    ...
    try {
      ...
      const response = await apiService.get<PaginatedResponse<Paycheck>>(endpointWithParams);
      setPaychecks(prevPaychecks => page === 1 ? response.data : [...prevPaychecks, ...response.data]);
  ...
  const renderPaycheckItem = ({ item }: { item: Paycheck }) => (
    <Card style={[localStyles.paycheckItem, { borderColor: getBorderColor(colorScheme) }]} >
      ...
      <ThemedView style={localStyles.itemRow}>
        ...
        <Text style={localStyles.itemValue}>{formatDateTime(item.createdAt)}</Text>
      </ThemedView>
    </Card>
  );

  const loadMorePaychecks = () => {
    if (paginationMeta && paginationMeta.currentPage < paginationMeta.lastPage && !loading) {
  ...
  if (!userId && !loading) {
    return (
      <ThemedView style={[globalStyles.container, styles.centered, { backgroundColor: getBackgroundColor(colorScheme) }]} >
        <Text style={globalStyles.title}>My Paychecks</Text>
        <Text style={{ color: getTextColor(colorScheme), marginTop: 20 }}>User information not available. Cannot load paychecks.</Text>
      </ThemedView>
    );
  }
  ...
  return (
    <ThemedView style={[globalStyles.container, { backgroundColor: getBackgroundColor(colorScheme) }]} >
      <Text style={[globalStyles.title, { color: getTextColor(colorScheme), marginBottom: 20 }]} >My Paychecks</Text>
      {loading && paychecks.length === 0 ? (
        <ActivityIndicator size="large" color={getTextColor(colorScheme)} style={styles.centered} />
      ) : error ? (
        <Text style={[styles.centeredText, { color: 'red' }]} >{error}</Text>
      ) : paychecks.length === 0 ? (
        <Text style={[styles.centeredText, { color: getTextColor(colorScheme) }]} >No paychecks found.</Text>
      ) : (
        <FlatList
          data={paychecks}
          ...
          ListFooterComponent={loading && paychecks.length > 0 ? <ActivityIndicator size="small" color={getTextColor(colorScheme)} style={{ marginVertical: 20 }} /> : null}
        />
      )}
    </ThemedView>
  );
