...
- **Document Picking:** `expo-image-picker` (for selecting files from storage)
...
- Refer to `package.json` for a full list of dependencies.
  ...
  - `expo-image-picker`: Allows users to select documents or files from the device's storage.
